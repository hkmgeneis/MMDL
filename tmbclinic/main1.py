...
from Net1 import Net, Cnn_With_Clinical_Net
...
def generative_model(model, k, cnv=True):   # 加入临床信息
    image_datasets = {x: CustomDset(os.getcwd()+f'/data/TMB20/{x}_{k}.csv',  # 按照交叉验证的划分数据集，生成csv文件，然后将数据导入进trian中。 改动
                        data_transforms[x]) for x in ['train']}  # 没有独立测试集，所有的都导入进trian中。

    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=64,  # 载入数据
    ...
    model_ft, tb = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, dataloaders, 
        dataset_sizes, num_epochs=20, cnv=cnv)
    tb.close()  # 释放空间。     tb：tensorboard

    save_model = os.getcwd()+f'/results/TMBC20/top5/{model}_{k}'  #用.pkl文件来保存模型的权重参数等。，改动
    if cnv:
...
def main(ocs, classification, K, cnv):
    ...
    for k in range(K):
        ...
        #path = os.getcwd()+f'/results/TMBC20/top5/resnet18_{k}'  # 路径是保存的模型的权重参数。
        #if cnv:
            #path = path + '_cnv'
