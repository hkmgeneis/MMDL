...
def test(model, model_name, k=0, K=10, types=0, cnv=True):
    ...
    if cnv:
        cnv_feature=pd.read_csv('/data/hkm/HEpreTMB/label/clinic/top5.csv')  # 改动1
    ...
    testset = CustomDset(f'/data/TMB20/test.csv', data_transforms['test'])  # 改动2
    ...
    score_list.to_csv(os.getcwd()+f'/results/TMBC20/top5/score_list.csv')
    preid_list.to_csv(os.getcwd()+f'/results/TMBC20/top5/preid_list.csv')
    
    np.save(os.getcwd()+f'/results/TMBC20/top5/y_true_{k}.npy', np.array(y_true))  # 改动地方
    np.save(os.getcwd()+f'/results/TMBC20/top5/preid_{k}.npy', np.array(preid_list))
    np.save(os.getcwd()+f'/results/TMBC20/top5/score_{k}.npy', np.array(score_list))
    np.save(os.getcwd()+f'/results/TMBC20/top5/y_pred_{k}.npy', np.array(y_pred))
